...
public:
    ...
    Spdz2GeneratorThread(Spdz2SetupThread<FD> &setup, Producer<FD> &producer, int id) : setup_thread(setup), producer(producer), thread(0), id(id)
    {
    }
...
{
public:
    PartSetup<FD> setup;        // Stores public and secret keys of the FHE scheme
    int plaintext_length;
    const Spdz2 &spdz2;
    Signal signal;

    // Initialized the SPDZ2Setup object
    Spdz2SetupThread(int plaintext_length, const Spdz2 &spdz2) : plaintext_length(plaintext_length), spdz2(spdz2)
    {
        signal.lock();
    }

    // Runs the offline phase - triple production etc
    void *run()
    {
...
int main(int argc, const char **argv)
{
    ...
    // Why make two threads for setup?
