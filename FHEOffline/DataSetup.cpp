...
template <class FD>
PartSetup<FD>::PartSetup() : pk(params, 0), sk(params, 0), calpha(params)
{
}
...
DataSetup::DataSetup() : params_p(setup_p.params), params_2(setup_2.params),
...
{
    sec = max(sec, 40);
    ::generate_setup(n_parties, plaintext_length, sec, params, FieldD, slack, round_up);
    params.set_sec(sec);
...
{
    if (params != other.params or FieldD != other.FieldD or pk != other.pk or sk != other.sk or calpha != other.calpha or alphai != other.alphai)
        return true;
    else
        return false;
}
